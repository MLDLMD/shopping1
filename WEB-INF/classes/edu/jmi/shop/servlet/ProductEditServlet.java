...
@WebServlet("/servlet/productedit")
public class ProductEditServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		out.println("<body>");
        //输出表单标签，
		out.println("<form name=\"form_productInfo\" action=\"" + request.getContextPath() + "/servlet/productaddpost\""
				+ " method=\"post\" >");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (sName==null   || sName.matches("^.{1,100}$")==false){
			request.getRequestDispatcher("/servlet/productedit?id=\""+sId).forward(request,response);
			return;
		}
		...
		if (b==true) {
			...
			response.sendRedirect(request.getContextPath()+"/servlet/productedit?id="+id);	
		}
}
}
